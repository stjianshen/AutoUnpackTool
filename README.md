...
- ✅ **智能密码测试**：
  ...
  - **增量测试**：避免对已测试文件的重复遍历
- ✅ **扩展名匹配**：
  - **可配置检测列表**：自定义哪些扩展名需要检测为压缩文件
  - **排除列表**：直接跳过不需要检测的文件类型（优先级最高）
  - **魔术数验证**：符合扩展名的文件会用文件头特征验证是否为真正的压缩文件
  - **智能检测流程**：排除 → 包含+验证 → 纯魔术数检测
- ✅ **实际解压功能**：集成 7-Zip 实现真实解压
- ✅ **软件设置**：可配置 7z.exe 路径、线程数、文件处理方式、扩展名匹配规则
- ✅ **密码本管理**：
